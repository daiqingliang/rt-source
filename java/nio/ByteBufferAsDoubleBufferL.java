...
class ByteBufferAsDoubleBufferL extends DoubleBuffer {
  ...
  public DoubleBuffer duplicate() { return new ByteBufferAsDoubleBufferL(this.bb, markValue(), position(), limit(), capacity(), this.offset); }
  
  public DoubleBuffer asReadOnlyBuffer() { return new ByteBufferAsDoubleBufferRL(this.bb, markValue(), position(), limit(), capacity(), this.offset); }
  
  protected int ix(int paramInt) { return (paramInt << 3) + this.offset; }
  
  public double get() { return Bits.getDoubleL(this.bb, ix(nextGetIndex())); }
  
  public double get(int paramInt) { return Bits.getDoubleL(this.bb, ix(checkIndex(paramInt))); }
