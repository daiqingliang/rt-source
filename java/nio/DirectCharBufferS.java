...
class DirectCharBufferS extends CharBuffer implements DirectBuffer {
  ...
  public CharBuffer duplicate() { return new DirectCharBufferS(this, markValue(), position(), limit(), capacity(), 0); }
  
  public CharBuffer asReadOnlyBuffer() { return new DirectCharBufferRS(this, markValue(), position(), limit(), capacity(), 0); }
  
  public long address() { return this.address; }
  
  private long ix(int paramInt) { return this.address + (paramInt << true); }
  
  public char get() { return Bits.swap(unsafe.getChar(ix(nextGetIndex()))); }
  
  public char get(int paramInt) { return Bits.swap(unsafe.getChar(ix(checkIndex(paramInt)))); }
  
  char getUnchecked(int paramInt) { return Bits.swap(unsafe.getChar(ix(paramInt))); }
  ...
  public CharBuffer put(CharBuffer paramCharBuffer) {
    if (paramCharBuffer instanceof DirectCharBufferS) {
      ...
      DirectCharBufferS directCharBufferS = (DirectCharBufferS)paramCharBuffer;
      int i = directCharBufferS.position();
      int j = directCharBufferS.limit();
      assert i <= j;
      int k = (i <= j) ? (j - i) : 0;
      int m = position();
      int n = limit();
      ...
      unsafe.copyMemory(directCharBufferS.ix(i), ix(m), k << true);
      directCharBufferS.position(i + k);
      position(m + k);
  ...
  public CharBuffer subSequence(int paramInt1, int paramInt2) {
    ...
    if (paramInt1 < 0 || paramInt2 > k || paramInt1 > paramInt2)
      throw new IndexOutOfBoundsException(); 
    return new DirectCharBufferS(this, -1, i + paramInt1, i + paramInt2, capacity(), this.offset);
  }
