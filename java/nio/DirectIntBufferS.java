...
class DirectIntBufferS extends IntBuffer implements DirectBuffer {
  ...
  public IntBuffer duplicate() { return new DirectIntBufferS(this, markValue(), position(), limit(), capacity(), 0); }
  
  public IntBuffer asReadOnlyBuffer() { return new DirectIntBufferRS(this, markValue(), position(), limit(), capacity(), 0); }
  
  public long address() { return this.address; }
  
  private long ix(int paramInt) { return this.address + (paramInt << 2); }
  
  public int get() { return Bits.swap(unsafe.getInt(ix(nextGetIndex()))); }
  
  public int get(int paramInt) { return Bits.swap(unsafe.getInt(ix(checkIndex(paramInt)))); }
  ...
  public IntBuffer put(IntBuffer paramIntBuffer) {
    if (paramIntBuffer instanceof DirectIntBufferS) {
      ...
      DirectIntBufferS directIntBufferS = (DirectIntBufferS)paramIntBuffer;
      int i = directIntBufferS.position();
      int j = directIntBufferS.limit();
      assert i <= j;
      int k = (i <= j) ? (j - i) : 0;
      int m = position();
      int n = limit();
      ...
      unsafe.copyMemory(directIntBufferS.ix(i), ix(m), k << 2);
      directIntBufferS.position(i + k);
      position(m + k);
