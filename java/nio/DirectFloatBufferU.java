...
class DirectFloatBufferU extends FloatBuffer implements DirectBuffer {
  ...
  public FloatBuffer duplicate() { return new DirectFloatBufferU(this, markValue(), position(), limit(), capacity(), 0); }
  
  public FloatBuffer asReadOnlyBuffer() { return new DirectFloatBufferRU(this, markValue(), position(), limit(), capacity(), 0); }
  
  public long address() { return this.address; }
  
  private long ix(int paramInt) { return this.address + (paramInt << 2); }
  
  public float get() { return unsafe.getFloat(ix(nextGetIndex())); }
  
  public float get(int paramInt) { return unsafe.getFloat(ix(checkIndex(paramInt))); }
  ...
  public FloatBuffer put(FloatBuffer paramFloatBuffer) {
    if (paramFloatBuffer instanceof DirectFloatBufferU) {
      ...
      unsafe.copyMemory(directFloatBufferU.ix(i), ix(m), k << 2);
      directFloatBufferU.position(i + k);
      position(m + k);
