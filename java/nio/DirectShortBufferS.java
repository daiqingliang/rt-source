...
class DirectShortBufferS extends ShortBuffer implements DirectBuffer {
  ...
  public ShortBuffer duplicate() { return new DirectShortBufferS(this, markValue(), position(), limit(), capacity(), 0); }
  
  public ShortBuffer asReadOnlyBuffer() { return new DirectShortBufferRS(this, markValue(), position(), limit(), capacity(), 0); }
  
  public long address() { return this.address; }
  
  private long ix(int paramInt) { return this.address + (paramInt << true); }
  
  public short get() { return Bits.swap(unsafe.getShort(ix(nextGetIndex()))); }
  
  public short get(int paramInt) { return Bits.swap(unsafe.getShort(ix(checkIndex(paramInt)))); }
  ...
  public ShortBuffer put(ShortBuffer paramShortBuffer) {
    if (paramShortBuffer instanceof DirectShortBufferS) {
      ...
      DirectShortBufferS directShortBufferS = (DirectShortBufferS)paramShortBuffer;
      int i = directShortBufferS.position();
      int j = directShortBufferS.limit();
      assert i <= j;
      int k = (i <= j) ? (j - i) : 0;
      int m = position();
      int n = limit();
      ...
      unsafe.copyMemory(directShortBufferS.ix(i), ix(m), k << true);
      directShortBufferS.position(i + k);
      position(m + k);
