...
class DirectLongBufferU extends LongBuffer implements DirectBuffer {
  ...
  public LongBuffer duplicate() { return new DirectLongBufferU(this, markValue(), position(), limit(), capacity(), 0); }
  
  public LongBuffer asReadOnlyBuffer() { return new DirectLongBufferRU(this, markValue(), position(), limit(), capacity(), 0); }
  
  public long address() { return this.address; }
  
  private long ix(int paramInt) { return this.address + (paramInt << 3); }
  
  public long get() { return unsafe.getLong(ix(nextGetIndex())); }
  
  public long get(int paramInt) { return unsafe.getLong(ix(checkIndex(paramInt))); }
  ...
  public LongBuffer put(LongBuffer paramLongBuffer) {
    if (paramLongBuffer instanceof DirectLongBufferU) {
      ...
      DirectLongBufferU directLongBufferU = (DirectLongBufferU)paramLongBuffer;
      int i = directLongBufferU.position();
      int j = directLongBufferU.limit();
      assert i <= j;
      int k = (i <= j) ? (j - i) : 0;
      int m = position();
      int n = limit();
      ...
      unsafe.copyMemory(directLongBufferU.ix(i), ix(m), k << 3);
      directLongBufferU.position(i + k);
      position(m + k);
