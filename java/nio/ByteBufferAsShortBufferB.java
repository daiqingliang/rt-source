...
class ByteBufferAsShortBufferB extends ShortBuffer {
  ...
  public ShortBuffer duplicate() { return new ByteBufferAsShortBufferB(this.bb, markValue(), position(), limit(), capacity(), this.offset); }
  
  public ShortBuffer asReadOnlyBuffer() { return new ByteBufferAsShortBufferRB(this.bb, markValue(), position(), limit(), capacity(), this.offset); }
  
  protected int ix(int paramInt) { return (paramInt << 1) + this.offset; }
  
  public short get() { return Bits.getShortB(this.bb, ix(nextGetIndex())); }
  
  public short get(int paramInt) { return Bits.getShortB(this.bb, ix(checkIndex(paramInt))); }
