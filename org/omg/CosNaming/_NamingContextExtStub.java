...
public class _NamingContextExtStub extends ObjectImpl implements NamingContextExt {
  ...
  public String to_string(NameComponent[] paramArrayOfNameComponent) throws InvalidName {
    inputStream = null;
    try {
      OutputStream outputStream = _request("to_string", true);
      NameHelper.write(outputStream, paramArrayOfNameComponent);
      inputStream = _invoke(outputStream);
      String str = StringNameHelper.read(inputStream);
      return str;
      ...
      return to_string(paramArrayOfNameComponent);
    } finally {
  ...
  public NameComponent[] to_name(String paramString) throws InvalidName {
    inputStream = null;
    try {
      OutputStream outputStream = _request("to_name", true);
      StringNameHelper.write(outputStream, paramString);
      inputStream = _invoke(outputStream);
      NameComponent[] arrayOfNameComponent = NameHelper.read(inputStream);
  ...
  public String to_url(String paramString1, String paramString2) throws InvalidAddress, InvalidName {
    inputStream = null;
    try {
      OutputStream outputStream = _request("to_url", true);
      AddressHelper.write(outputStream, paramString1);
      StringNameHelper.write(outputStream, paramString2);
      inputStream = _invoke(outputStream);
      String str = URLStringHelper.read(inputStream);
      return str;
    } catch (ApplicationException applicationException) {
      inputStream = applicationException.getInputStream();
      String str = applicationException.getId();
      if (str.equals("IDL:omg.org/CosNaming/NamingContextExt/InvalidAddress:1.0"))
        throw InvalidAddressHelper.read(inputStream); 
      if (str.equals("IDL:omg.org/CosNaming/NamingContext/InvalidName:1.0"))
        throw InvalidNameHelper.read(inputStream); 
      throw new MARSHAL(str);
    } catch (RemarshalException remarshalException) {
      return to_url(paramString1, paramString2);
    } finally {
      _releaseReply(inputStream);
    } 
  }
  ...
  public Object resolve_str(String paramString) throws NotFound, CannotProceed, InvalidName {
    inputStream = null;
    try {
      OutputStream outputStream = _request("resolve_str", true);
      StringNameHelper.write(outputStream, paramString);
      inputStream = _invoke(outputStream);
      ...
      return resolve_str(paramString);
    } finally {
      _releaseReply(inputStream);
    } 
  }
...
/* Location:              D:\software\jd-gui\jd-gui-windows-1.6.3\rt.jar!\org\omg\CosNaming\_NamingContextExtStub.class
