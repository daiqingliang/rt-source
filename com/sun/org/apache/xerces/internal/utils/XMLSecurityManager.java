...
public final class XMLSecurityManager {
  ...
  public boolean isSecureProcessing() { return this.secureProcessing; }
  ...
  public boolean isNoLimit(int paramInt) { return (paramInt == 0); }
  
  public boolean isOverLimit(Limit paramLimit, String paramString, int paramInt, XMLLimitAnalyzer paramXMLLimitAnalyzer) { return isOverLimit(paramLimit.ordinal(), paramString, paramInt, paramXMLLimitAnalyzer); }
  ...
  public boolean isOverLimit(Limit paramLimit, XMLLimitAnalyzer paramXMLLimitAnalyzer) { return isOverLimit(paramLimit.ordinal(), paramXMLLimitAnalyzer); }
  
  public boolean isOverLimit(int paramInt, XMLLimitAnalyzer paramXMLLimitAnalyzer) { return (this.values[paramInt] == 0) ? false : ((paramInt == Limit.ELEMENT_ATTRIBUTE_LIMIT.ordinal() || paramInt == Limit.ENTITY_EXPANSION_LIMIT.ordinal() || paramInt == Limit.TOTAL_ENTITY_SIZE_LIMIT.ordinal() || paramInt == Limit.ENTITY_REPLACEMENT_LIMIT.ordinal() || paramInt == Limit.MAX_ELEMENT_DEPTH_LIMIT.ordinal() || paramInt == Limit.MAX_NAME_LIMIT.ordinal()) ? ((paramXMLLimitAnalyzer.getTotalValue(paramInt) > this.values[paramInt])) : ((paramXMLLimitAnalyzer.getValue(paramInt) > this.values[paramInt]))); }
  ...
  public static XMLSecurityManager convert(Object paramObject, XMLSecurityManager paramXMLSecurityManager) {
    ...
    if (SecurityManager.class.isAssignableFrom(paramObject.getClass())) {
      SecurityManager securityManager = (SecurityManager)paramObject;
      paramXMLSecurityManager.setLimit(Limit.MAX_OCCUR_NODE_LIMIT, State.APIPROPERTY, securityManager.getMaxOccurNodeLimit());
      paramXMLSecurityManager.setLimit(Limit.ENTITY_EXPANSION_LIMIT, State.APIPROPERTY, securityManager.getEntityExpansionLimit());
      paramXMLSecurityManager.setLimit(Limit.ELEMENT_ATTRIBUTE_LIMIT, State.APIPROPERTY, securityManager.getElementAttrLimit());
    } 
    return paramXMLSecurityManager;
  }
...
/* Location:              D:\software\jd-gui\jd-gui-windows-1.6.3\rt.jar!\com\sun\org\apache\xerces\interna\\utils\XMLSecurityManager.class
